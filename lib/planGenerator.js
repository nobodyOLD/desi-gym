...
export function generateWorkoutPlanLocal(profile) {
  ...
  const buildDailyExercises = (prescriptions, yesterdayExercises = new Set(), weeklyExercises = new Set(), dailyRedundancyGroups = new Set()) => {
    ...
    for (const presc of prescriptions) {
      ...
      const candidates = EXERCISE_DATABASE.map(ex => {
        ...
        // 0. User Exclusion Check (Hard filter)
        if (excluded.includes(ex.id)) {
        ...
        // 3.5. User Favorite Match (+30 pts)
  ...
  const getPowerOrStandardPattern = (stdPattern, stdMuscle, powerId) => {
    ...
    // 6. Program Splits Architecture
  const availableDays = (profile.metadata && profile.metadata.available_days) || ['monday', 'wednesday', 'friday'];
  const splitPref = (profile.metadata && profile.metadata.split_preference) || 'recommended';
  ...
  const getTemplates = () => {
    ...
    const fbD = {
      name: "Full Body D",
      muscle_group: "Shoulders, Legs & Arms Focus",
      prescriptions: [
        { pattern: "vertical_push", muscle: "front_delts" },
    ...
    const pushStr = {
      ...
      prescriptions: [
        { pattern: "horizontal_push", sets: 4, reps: "6-8", purpose: "primary horizontal push" },
        { pattern: "vertical_push", sets: 3, reps: "8-10", purpose: "primary vertical push" },
        { pattern: "horizontal_push", sets: 3, reps: "10-12", purpose: "secondary horizontal push" },
        { pattern: "shoulder_abduction", sets: 3, reps: "12-15", purpose: "side delt isolation" },
    ...
    const pullStr = {
      ...
      prescriptions: [
        { pattern: "vertical_pull", sets: 4, reps: "6-8", purpose: "primary vertical pull" },
        { pattern: "horizontal_pull", sets: 4, reps: "8-10", purpose: "primary horizontal pull" },
        { pattern: "rear_delt_isolation", sets: 3, reps: "12-15", purpose: "rear delt isolation" },
        { pattern: "elbow_flexion", sets: 3, reps: "8-10", purpose: "biceps primary" },
        { pattern: "elbow_flexion", sets: 2, reps: "10-12", purpose: "biceps secondary" }
      ]
    };
    const legsStr = {
      name: "Legs Focus",
      ...
      prescriptions: [
        { pattern: "knee_dominant", sets: 4, reps: "6-8", purpose: "primary knee dominant" },
        ...
        { pattern: "knee_dominant", muscle: "quads", sets: 3, reps: "12-15", purpose: "quad isolation" },
        { pattern: "calf_plantar_flexion", sets: 4, reps: "12-15", purpose: "calf volume" }
      ]
    };

    return { fbA, fbB, fbC, fbD, upperA, lowerA, upperB, lowerB, pushStr, pullStr, legsStr };
  };

  const templates = getTemplates();

  const determineOptimalSplit = (daysCount, exp, goal) => {
    if (daysCount <= 3) return 'full_body';
    if (daysCount === 4) return 'upper_lower';
    if (daysCount === 5) return 'upper_lower_ppl';
    return 'ppl'; // 6 or 7 days
  };

  const actualSplit = splitPref === 'recommended' ? determineOptimalSplit(availableDays.length, fitnessLevel, fitnessGoal) : splitPref;

  let splitBlocks = [];
  if (actualSplit === 'full_body') {
    splitBlocks = [templates.fbA, templates.fbB, templates.fbC, templates.fbD];
  } else if (actualSplit === 'upper_lower') {
    splitBlocks = [templates.upperA, templates.lowerA, templates.upperB, templates.lowerB];
  } else if (actualSplit === 'ppl') {
    splitBlocks = [templates.pushStr, templates.pullStr, templates.legsStr, templates.pushStr, templates.pullStr, templates.legsStr];
  } else if (actualSplit === 'upper_lower_ppl') {
    splitBlocks = [templates.upperA, templates.lowerA, templates.pushStr, templates.pullStr, templates.legsStr];
  } else {
    splitBlocks = [templates.fbA, templates.fbB, templates.fbC];
  }

  // Adjust blocks length based on available days
  let finalBlocks = [];
  for (let i = 0; i < availableDays.length; i++) {
    finalBlocks.push(splitBlocks[i % splitBlocks.length]);
  }
  ...
  for (const day of weekdayKeys) {
    if (availableDays.includes(day) && blockIndex < finalBlocks.length) {
      baseSchedule[day] = finalBlocks[blockIndex];
      blockIndex++;
    } else {
      baseSchedule[day] = { type: 'rest', activities: ['Light walk', 'Stretching'] };
    }
  }
  ...
  const generateWhyThisWorkout = () => {
    let rationale = `This ACSM-informed program is optimized for your ${fitnessLevel} experience level and ${availableDays.length}-day training frequency. `;
    if (fitnessGoal === 'strength') {
